...
INT8U  MSGstr[400];
INT8U  tempp[300];   //函数内部不能放太大的数组，否则单片机会死机！
...
{ 
   ...
   StrCat(MSGstr,(char *)"91");
   GSM_InvertNumbers(tgPhone,Target_phone);
   StrCat(MSGstr,tgPhone);
   StrCat(MSGstr,(char *)"000801");
   GB2UnicodeStr(tempp,Content); //短信内容转成Unicode字符串，放到strMSG[]
   I2HEXa8(strLEN,strlen((char *)tempp)/2); //转成十六进制形式的字符串
   StrCat(MSGstr,strLEN);
   StrCat(MSGstr,tempp);
   StrCat(MSGstr,pEND);
   IWDG_ReloadCounter();//喂狗
...
{
	char WMTempData[50], strTemp[10], str[13];
	WMTempData[0] = '\0';
...
▇ 调用示例：GSM_Send_CN_MSG("8615990472896","你好！Hellow World!");							  ▇
▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇*/
void GPRS_CopyWMData(char* Buffer, WM_Data *WMData)
{
	int j;
	char strTemp[37] = {0};
